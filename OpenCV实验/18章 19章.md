...
欲投影的点位于一些刚体上，这些点的位置可以在以物体自身为坐标原点的坐标系上表示出来，而不是在摄像机坐标系中的位置，之后通过加入旋转和平移来定义物体坐标系和摄像机坐标系之间的关系。实际上cv::projectPoints()函数在cv::calibrateCamera()中被内部调用，

第一个参数objectPoints，是**需要投影的点序列**，它可以是任何常见的格式：Nx3，3xN，Nx1或1xN 的cv::Vec3f 对象数组，或者仅是一个单纯的旧STL类型向量，其中包含**点的位置**。将这些带入物体自身坐标系，之后可以得到旋转矢量和平移矢量，将物体坐标系和摄像机坐标系联系起来。如果情况特殊，直接在摄像机坐标系中计算更为简单，那么可以只设置该坐标系中的objectpoints ，并把旋转矢量和平移矢量内的元素设置为0